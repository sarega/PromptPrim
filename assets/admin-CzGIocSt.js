...
`);return t+n}async function un(e){const t=E(e)||S(e);if(!t){alert("No activity to export.");return}if(M(t)){const i=await Re(t,{limit:500});if(i.length===0){alert("No activity to export.");return}const o=i.map(u=>({timestamp:u.timestamp,model:u.model,promptTokens:u.promptTokens,completionTokens:u.completionTokens,costUSD:u.providerCostUSD,duration:0})),c=ke(o),p=new Blob([c],{type:"text/csv;charset=utf-8;"}),l=URL.createObjectURL(p),d=document.createElement("a");d.setAttribute("href",l),d.setAttribute("download",`activity_log_${t.userName}_${new Date().toISOString().slice(0,10)}.csv`),document.body.appendChild(d),d.click(),document.body.removeChild(d);return}if(!t.activityLog||t.activityLog.length===0){alert("No activity to export.");return}const n=ke(t.activityLog),a=new Blob([n],{type:"text/csv;charset=utf-8;"}),s=URL.createObjectURL(a),r=document.createElement("a");r.setAttribute("href",s),r.setAttribute("download",`activity_log_${t.userName}_${new Date().toISOString().slice(0,10)}.csv`),document.body.appendChild(r),r.click(),document.body.removeChild(r)}const ae=document.getElementById("activity-log-modal"),P=document.getElementById("activity-log-body"),we=document.getElementById("activity-log-title");let ge=null;function K(){ae.style.display="flex"}function mn(){ae.style.display="none"}async function Ze(e){ge=e;const t=E(e)||S(e);if(!t||!P||!we)return;if(we.textContent=`Activity Log for ${t.userName}`,M(t))try{const a=await Re(t,{limit:100});if(a.length===0){P.innerHTML="<p>No backend activity recorded for this user.</p>",K();return}const s=a.map(r=>`
                <tr>
                    <td>${A(r.timestamp)}</td>
                    <td>${r.model}</td>
