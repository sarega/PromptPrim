...
export async function sendMessage(isRegeneration = false) {
    ...
    if (type === 'group') {
        ...
        if (group.flowType === 'round-robin') {
            await handleGroupChatRoundRobin(project, session, group);
        } else if (group.flowType === 'auto-moderator') {
            await handleGroupChatAutoModerator(project, session, group);
        } else {
            showCustomAlert(`Unknown group flow type: ${group.flowType}`, 'Error');
        }
    } else {
