// ===============================================
// FILE: src/js/modules/chat/chat.handlers.js (แก้ไขแล้ว)
// DESCRIPTION: เพิ่ม Logic การทำงานของ Summarize
// ===============================================
...
/**
 * [NEW] Core logic for creating a conversation summary.
 */
async function runSummarization() {
    ...
    try {
        ...
        if (!utilityAgent || !utilityAgent.model) {
            throw new Error("System Utility Model for summarization is not configured in settings.");
        }

        const summarizedUntil = session.summaryState?.summarizedUntilIndex || 0;
        const messagesToSummarize = session.history.slice(summarizedUntil);
        if (messagesToSummarize.length < 2) {
            throw new Error("Not enough new messages to summarize.");
        }

        const previousSummary = session.summaryState?.activeSummaryId
            ? project.summaryLogs.find(l => l.id === session.summaryState.activeSummaryId)?.content || 'None.'
            : 'None.';

        const newMessagesText = messagesToSummarize.map(m => `${m.role}: ${Array.isArray(m.content) ? m.content.find(p=>p.type==='text')?.text || '[Image]' : m.content}`).join('\n');
        
        let promptTemplate = utilityAgent.summarizationPrompt || "Summarize the following conversation. Previous summary: ${previousSummary}\nNew messages:\n${newMessages}";
        const prompt = promptTemplate.replace('${previousSummary}', previousSummary).replace('${newMessages}', newMessagesText);
        
        const summaryText = await callLLM(utilityAgent, [{ role: 'user', content: prompt }]);

        const newLog = {
            id: `sum_${Date.now()}`,
            timestamp: Date.now(),
            summary: `Summary of ${messagesToSummarize.length} new messages`,
            content: summaryText,
        ...
        session.summaryState.activeSummaryId = newLog.id;
        session.summaryState.summarizedUntilIndex = newLog.summarizedUntilIndex;
...
/**
 * [MODIFIED] Replaced placeholder with a call to the new core function.
 */
...
/**
 * [NEW] Handler to load a specific summary into the current chat context.
 * @param {string} logId - The ID of the summary log to load.
 */
export async function loadSummaryIntoContext(logId) {
    ...
    session.summaryState.summarizedUntilIndex = log.summarizedUntilIndex;
    
    const systemMessage = { role: 'system', content: `[ System: Loaded context from summary dated ${new Date(log.timestamp).toLocaleTimeString()} ]` };
    ...
    stateManager.bus.publish('ui:renderChatMessages');
    stateManager.bus.publish('summary:listChanged'); // Re-render logs to show active one
}
...
export function regenerateMessage(index) {
    ...
    stopGeneration();
    let lastUserIndex = -1;
    for (let i = index; i >= 0; i--) { // Search backwards from the target index
